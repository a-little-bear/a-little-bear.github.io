...
$.getJSON("../json_data/music.json", function (data) {
    ...
    for (var i = 0; i < musicJsons.length; i++) {
        ...
        var $li = $('<li><span>' + musicJsons[i].title + '</span>&nbsp;&nbsp;&nbsp;&nbsp;' + (_isEn ? 'Duration: ' : '时长：') + musicJsons[i].time + '&nbsp;&nbsp;&nbsp;&nbsp; ' + (_isEn ? 'Artist: ' : '歌手：') + musicJsons[i].author + '</li>');
        $li.attr('id', i);
...
function playVideo(data,id) {
    new DPlayer({
        ...
        theme: '#42b983',
        loop: 'false' === 'true',
        lang: (window._getLang && window._getLang() === 'en') ? 'en' : 'zh-cn',
        preload: 'none',
        volume: Number('0.7'),
        video: {
            url: data['url'],
