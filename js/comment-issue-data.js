...
function dealWtihContentStr(contentStr) {
    var _isEn = window._getLang && window._getLang() === 'en';
    if (contentStr == undefined || contentStr == "") {
        contentStr = _isEn ? "Empty content!" : "内容为空！";
    }

    // 替换图片
    contentStr = contentStr.replace(/![\s\w\](?:http(s)?:\/\/)+[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\]@!\$&'\*\+,;=.]+\)/g, _isEn ? "[Image]" : "[图片]");

    // 替换网址
    contentStr = contentStr.replace(/(?:http(s)?:\/\/)+[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\]@!\$&'\*\+,;=.]+/g, _isEn ? "[Link]" : "[网址]");
...
function addCommentInfo(result, resultArr, item, endIndex, i, contentStr) {
    ...
    if (userName != undefined && userName != '' && userName == ADMIN_NAME) {
        userName += (window._getLang && window._getLang() === 'en') ? '[Admin]' : '[博主]';
    }
    resultArr.push({
        "content": contentStr,
...
function renderCommentData(COMMENT_ARR) {
    if (COMMENT_ARR.length > 0) {
        ...
        for (var i = 0; i < COMMENT_ARR.length; i++) {
            ...
            htmlContentWidget +=
                "<div class='card-comment-item'>" + 
                    ...
                    "<div class='item-header-text'><a href='" + item.userUrl + "' target='_blank'>" + item.userName + "</a>&nbsp;" + ((window._getLang && window._getLang() === 'en') ? "posted" : "发表于") + getDateDiff(new Date(item.date).getTime()) + "</div>" + "<div class=\"item-text\"><a href =\"" + item.url + '#comment-container\">' + contentStr + "</a></div>" +
                "</div>";
        }
        ...
        $(".body_hot_comment").html((window._getLang && window._getLang() === 'en') ? "No data!" : "无数据记录！");
    }
}
...
function loadIndexHotData(authorizationToken) {
    ...
    if ($("#index_hot_div").length > 0) {
        ...
        ajaxReqForGitHub(repoIssuesUrl + "?per_page=10&sort=comments", authorizationToken, function (result) {
            ...
            hotDiv.html("");
            if (hotContent == "") {
                hotDiv.append((window._getLang && window._getLang() === 'en') ? "No data!" : "无数据记录！");
            } else {
    ...
    if ($(".body_hot_comment").length > 0) {
        ...
        valine.Q('*').limit(8).find().then(function (comments) {
            ...
            for (var i = 0; i < comments.length; i++) {
                ...
                if (nick == ADMIN_NAME) {
                    nick += (window._getLang && window._getLang() === 'en') ? '[Admin]' : '[博主]';
                }
...
function loadIssueData(appId, appKey, userName, userRepo, isValine) {
    ...
    setTimeout(function () { // 延迟1s执行，保证其余的先加载
            ...
            if (typeof Valine == "function") {
                ...
                if (valine == undefined || valine == null) {
                    valine = new Valine({
                        ...
                        appId: appId,
                        appKey: appKey,
                        placeholder: (window._getLang && window._getLang() === 'en') ? 'Leave a comment!' : '留下您的高见！',
                        avatar: 'mp',
                        avatarForce: false,
                        meta: ["nick", "mail", "link"],
                        pageSize: 10,
        ...
        // console.clear();
        console.log((window._getLang && window._getLang() === 'en') ? "~~~~Welcome! Come back often! ~~~~" : "~~~~欢迎光临！记得有时间多来看看哦 ~~~~")
    }
        ,
        500
    )
